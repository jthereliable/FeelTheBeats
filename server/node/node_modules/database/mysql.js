...
exports = module.exports = (function() {
	...
	that.escape = function(s) {
		return mysql.escape(s);
	};
	that.escapeId = function(s) {
		return mysql.escapeId(s);
	};
	that.escapeAlias = function(s) {
		return mysql.escapeId(s, true);
	};
	
	that.escapeArray = function(a) {
		var i, l=a.length, o=[];
		for(i=0;i<l;i++)
		{
			o[i] = mysql.escape(a[i]);
		}
		return o;
	};
	that.escapeIdArray = function(a) {
		var i, l=a.length, o=[];
		for(i=0;i<l;i++)
		{
			o[i] = mysql.escapeId(a[i]);
		}
		return o;
	};
	
	var questionMarks = [];
	that.questionMarks = function(n) {
		var l=questionMarks.length;
		for(l;l<n;l++)
		{
			questionMarks.push("?");
		}
	...
	var questionMarksId = [];
	that.questionMarksId = function(n) {
		var l=questionMarksId.length;
		for(l;l<n;l++)
		{
	...
	that.query = function(query, args, callback) {
		...
		that.acquire(function(connection) {
			...
			var q = connection.query(query, args, function(err, rows) {
				that.release(connection);
				if(err)
				{
					err.sql = q.sql;
