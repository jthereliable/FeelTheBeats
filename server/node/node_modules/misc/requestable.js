...
collection.get = function(req, from, where, fields, limit, callback) {
		...
		if(f.tables)
		{
			...
			for(j=0;j<jl;j++)
			{
				...
				if(__.isArray(t.table))
				{
					...
					{
						sql.left_join(
							mysql.escapeId(t.table[0]),
							mysql.escapeId(t.table[1]),
	...
	async.parallel([
		...
		function(next) {
			mysql.query(count_sql, function(err, rows) {
				if(err)
				{
					next(err);
					return;
...
model.get = function(req, from, where, fields, callback) {
	...
	for(i=0;i<l;i++)
	{
		k = keys[i];
		f = fields[k];
		if(!f.unignoreable && ((f.ignore && allowed.indexOf(k) == -1) || ignore.indexOf(k) != -1 || (k.indexOf(".")>-1 && ignore.indexOf(k.split(".",1)[0]) != -1)))
		{
			continue;
		}
		if(f.collection)
